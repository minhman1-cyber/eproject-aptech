...
const ENDPOINTS = {
    doctors: API_BASE_URL + 'admin_doctor_manager.php',
    patients: API_BASE_URL + 'admin_patient_manager.php',
    appointments: API_BASE_URL + 'admin_appointment_manager.php' 
};
...
const StatCard = ({ title, value, subtext, icon, color, isLoading }) => (
  <div className="card border-0 shadow-sm h-100 feature-box m-0">
    <div className="card-body">
      ...
      {isLoading ? (
        <div className="spinner-border spinner-border-sm text-secondary mb-1" role="status">
...
const DashboardHome = () => {
    ...
    const [stats, setStats] = useState({
        ...
        patients: 0,
        appointmentsToday: 0,
        revenue: '450M' // Giữ nguyên doanh thu cứng
    });
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            setIsLoading(true);
            try {
                // CẤU HÌNH QUAN TRỌNG: credentials: 'include' để gửi Cookie Session đi kèm
                const fetchOptions = {
                    method: 'GET',
                    headers: {
                ...
                // Gọi API song song với fetchOptions
                const [docsRes, patsRes, apptsRes] = await Promise.allSettled([
                    fetch(ENDPOINTS.doctors, fetchOptions),
                    fetch(ENDPOINTS.patients, fetchOptions),
                    fetch(ENDPOINTS.appointments, fetchOptions)
                ]);
                ...
                // 2. Xử lý số lượng Bệnh nhân
                if (patsRes.status === 'fulfilled' && patsRes.value.ok) {
                    const data = await patsRes.value.json();
                    const list = data.data?.patients || data.data || [];
                    patientCount = Array.isArray(list) ? list.length : 0;
                }

                // 3. Xử lý Lịch hẹn hôm nay
                if (apptsRes.status === 'fulfilled' && apptsRes.value.ok) {
                    ...
                    if (Array.isArray(list)) {
                        // Lọc các lịch có ngày trùng hôm nay (dựa trên field appointment_date hoặc date)
                        todayApptCount = list.filter(appt => {
                ...
                setStats(prev => ({
                    ...
                    appointmentsToday: todayApptCount
                }));
    ...
    return (
      <div className="fade-in">
        ...
        <div className="row g-4">
          {/* Cột trái: Widget Quản lý Bài viết */}
          <div className="col-lg-7">
             <AdminArticleManager isWidget={true} />
          </div>

          {/* Cột phải: Widget Duyệt bằng cấp & Hỗ trợ */}
...
const AdminDashboard = () => {
  ...
  // Render Content với các Component thật
  const renderContent = () => {
    switch (activeTab) {
      case 'dashboard': return <DashboardHome />;
      case 'articles': return <div className="fade-in"><AdminArticleManager /></div>;
      case 'doctors': return <div className="fade-in"><AdminDoctorManager /></div>;
      case 'patients': return <div className="fade-in"><AdminPatientManager /></div>;
      case 'cities': return <div className="fade-in"><AdminCityManager /></div>;
      case 'degrees': return <div className="fade-in"><AdminQualificationManager /></div>;
  ...
  return (
    <div className="d-flex h-100" style={{ minHeight: '100vh', fontFamily: "'Open Sans', sans-serif", backgroundColor: 'var(--bg-light)' }}>
      <style>{`
        ...
        body { font-family: 'Open Sans', sans-serif; color: var(--text-color); overflow-x: hidden; }
        ...
        .feature-box { background: #fff; transition: transform 0.3s; }
      ...
      <div className="flex-grow-1 d-flex flex-column" style={{ maxHeight: '100vh', overflow: 'hidden' }}>
        {isMobile && (
          <nav className="navbar navbar-light bg-white border-bottom px-3 flex-shrink-0">
             ...
             <button className="btn btn-sm btn-outline-secondary"><i className="fas fa-bars"></i></button>
          </nav>
        )}
